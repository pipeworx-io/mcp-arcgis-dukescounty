 ...
 *
 * Dukes County GIS (ArcGIS org FNsEJ848HT5uDOHD): parcels, addresses, zoning & public works. search_datasets finds layers (each
...
const UA = 'pipeworx-mcp-arcgis-dukescounty/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Dukes County GIS open geospatial datasets (parcels, addresses, zoning & public works) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (Dukes County GIS).' },
